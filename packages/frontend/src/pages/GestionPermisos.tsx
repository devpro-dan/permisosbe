          ...
          <div className="px-4 pb-4 space-y-3 border-t pt-3">
            ...
            {previewData && (
              <div className="space-y-3">
                <div className="flex items-center justify-between"><p className="text-xs font-medium text-gray-700">Previsualización: {previewData.total} filas — <span className="text-green-600">{previewData.preview.filter((r) => r.valido).length} válidas</span> / <span className="text-red-600">{previewData.errors.length} con errores</span></p><div className="flex gap-2"><button onClick={() => { setPreviewData(null); setPreviewFile(null); }} className="px-3 py-1 text-xs border rounded-lg hover:bg-gray-50">Descartar</button><button onClick={handleConfirmImport} disabled={importLoading || previewData.preview.filter((r) => r.valido).length === 0} className="px-4 py-1.5 text-xs bg-primary-600 text-white rounded-lg hover:bg-primary-700 disabled:opacity-50">{importLoading ? 'Importando...' : 'Confirmar importación'}</button></div></div>
